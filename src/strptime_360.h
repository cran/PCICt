...
day_of_the_year (struct tm *tm)
{
    ...
    tm->tm_yday = (__mon_yday[tm->tm_mon]
		   + (tm->tm_mday - 1));
}
		     ...
		     int *poffset)
{
	...
	if ( !(have_mon && have_mday) && have_yday)  {
	    ...
	    int t_mon = 0;
	    while (__mon_yday[t_mon] <= tm->tm_yday)
		t_mon++;
	    if (!have_mon)
		tm->tm_mon = t_mon - 1;
	    if (!have_mday)
		tm->tm_mday = (tm->tm_yday - __mon_yday[t_mon - 1] + 1);
	}
	day_of_the_week (tm);
    }
...
static char *
strptime_360(const char *buf, const char *format, struct tm *tm, 
